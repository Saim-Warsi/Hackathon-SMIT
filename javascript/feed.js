...
function renderPosts() {
  ...
  filteredPosts.forEach((post) => {
    ...
    const reactionsHtml = post.reactions
      ? Object.keys(post.reactions)
          .map(
            (emoji) => `
          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-sm bg-gray-100 dark:bg-gray-700 select-none cursor-default">
            <span class="material-symbols-outlined text-[16px] text-gray-600 dark:text-gray-300">${emoji}</span>
  ...
  <div class="flex justify-between items-start mb-3">
      <div>
          <p class="font-bold text-gray-800 dark:text-white">${post.author}</p>
      ...
      ${
        post.authorEmail === currentUser.email
          ? `<div class="flex gap-2">
              <button onclick="openEditModal(${post.id})" class="text-blue-500 hover:text-blue-600 text-sm font-semibold">
  ...
  <div class="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-gray-700">
    ...
    <button
      ...
      class="flex items-center justify-center p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition cursor-pointer"
    >
