 tailwind.config = { darkMode: 'class' };
        ...
        function loadProfile() {
            ...
            if (currentUser.joinedDate) {
                ...
                document.getElementById('joinedDate').textContent = 'Joined ' + joinDate.toLocaleDateString();
            }
        ...
        function renderUserPosts(userPosts) {
            ...
            if (userPosts.length === 0) {
                container.innerHTML = '<p class="text-center text-gray-500 dark:text-gray-400 py-8">No posts yet</p>';
                return;
            }
            ...
            userPosts.forEach(post => {
                ...
                postDiv.className = 'bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700';
                
                const date = new Date(post.timestamp);
                const formattedDate = date.toLocaleDateString() + ' ' + date.toLocaleTimeString();

                const reactionsCount = post.reactions ? Object.values(post.reactions).flat().length : 0;

                postDiv.innerHTML = `
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">${formattedDate}${post.edited ? ' • Edited' : ''}</p>
                    <p class="text-gray-800 dark:text-white whitespace-pre-wrap">${post.text}</p>
                    ${post.image ? `<img src="${post.image}" class="w-full rounded-lg mt-3 shadow-md" style="max-height: 400px; object-fit: cover;">` : ''}
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-3">${reactionsCount} reaction${reactionsCount !== 1 ? 's' : ''}</p>
                `;
